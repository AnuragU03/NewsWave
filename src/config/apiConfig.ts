...
function getApiKeys(envVar: string | undefined, placeholder: string): string[] {
  // console.log(`[getApiKeys] Checking envVar for placeholder "${placeholder}": Value is "${envVar}"`); // DEBUG LOG - Removed for prod
...
function getMultipleApiKeys(envVars: (string | undefined)[], placeholders: string[]): string[] {
  ...
  envVars.forEach((key, index) => {
    ...
    // console.log(`[getMultipleApiKeys] Checking envVar for placeholder "${placeholder}": Value is "${key}"`); // DEBUG LOG - Removed for prod
